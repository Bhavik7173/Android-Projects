...
public class Recycler_View_Skills extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<ArrayList<AllSkillData>> call = api.getAllSkillData(id);

        call.enqueue(new Callback<ArrayList<AllSkillData>>() {
            @Override
            public void onResponse(Call<ArrayList<AllSkillData>> call, Response<ArrayList<AllSkillData>> response) {
                ar = response.body();
                RCAdapter_S ar1 = new RCAdapter_S(ar);
                rc.setAdapter(ar1);
                Log.d("Skill Data:",response.body().toString());
            }

            @Override
            public void onFailure(Call<ArrayList<AllSkillData>> call, Throwable t) {
