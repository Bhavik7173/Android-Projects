...
public class FragmentHome extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        Call<ArrayList<HomeResponse>> call;
        Api api = r.create(Api.class);
        call = api.getHomeData();
        Log.d("login","in home fragment");
        call.enqueue(new Callback<ArrayList<HomeResponse>>() {
            ...
            public void onResponse(Call<ArrayList<HomeResponse>> call, Response<ArrayList<HomeResponse>> response) {
                /*Log.d("login","hii in call");
                ar = response.body();
                Log.d("login",ar.toString());
                if (!ar.isEmpty()) {
    ...
    public void CreateLeftCardViewProgrammatically(final String text, final String url, LinearLayout layout) {
        ...
        Log.d("myhubs",this.getString(R.string.IP1)+url);
        Glide.with(linearLayout.getContext()).load(this.getString(R.string.IP1)+url).error(R.drawable.home_static_image).diskCacheStrategy(DiskCacheStrategy.NONE).skipMemoryCache(true).listener(new RequestListener<Drawable>() {
            @Override
