...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        if (!cursor.moveToPosition(position))
            return;
        String guestName = cursor.getString(cursor.getColumnIndex(StudentModel.WaitListEntry.GUEST_NAME));
        int partySize = cursor.getInt(cursor.getColumnIndex(StudentModel.WaitListEntry.PARTY_SIZE));
        float id = cursor.getFloat(cursor.getColumnIndex(StudentModel.WaitListEntry._ID));
